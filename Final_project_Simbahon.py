...
class VehicleCapacityError(Exception): pass
...
class Location:
    ...
    def distance_to(self, other: "Location") -> float:
        ...
        R = 6371.0
        p1 = math.radians(self.__lat)
        p2 = math.radians(other.lat)
        dp = math.radians(other.lat - self.__lat)
        dl = math.radians(other.lon - self.__lon)
        a  = math.sin(dp/2)**2 + math.cos(p1)*math.cos(p2)*math.sin(dl/2)**2
        return R * 2 * math.atan2(math.sqrt(a), math.sqrt(1 - a))

    def __str__(self):  return f"{self.__name} ({self.__loc_type})"
    def __repr__(self): return f"Location('{self.__name}', {self.__lat}, {self.__lon})"
...
class Vehicle(ABC):
    ...
    def __init__(self, origin: Location, destination: Location):
        ...
        self._direction   = 1
        self._active      = True
        self._pax         = random.randint(0, self.capacity // 2)
    ...
    @property
    @abstractmethod
    def vehicle_type(self) -> str: pass
    ...
    def occupancy_pct(self) -> float:
        return self._pax / self.capacity
    ...
    def tick(self):
        ...
        if self._progress >= 1.0:
            ...
            self._progress  = 0.0
            self._direction = 1
            self._pax = random.randint(0, self.capacity)
        # Small random passenger change
        if random.random() < 0.12:
            new_pax = max(0, min(self.capacity, self._pax + random.randint(-2, 2)))
            self._pax = new_pax
    ...
    def get_info(self) -> dict:
        return {
            ...
            "Passengers": f"{self._pax}/{self.capacity}",
    ...
    def __str__(self):
        return (f"{self.get_icon()} {self._id} | {self.vehicle_type} | "
                ...
                f"{self._pax}/{self.capacity}")

class Jeepney(Vehicle):
    def __init__(self, origin, destination): super().__init__(origin, destination)
    @property
...
class User:
    ...
    def __str__(self): return f"User({self.__username}, role={self.__role})"

class Booking:
    """Association between User and Vehicle."""
    _booking_counter = 0
    ...
    def cancel(self):     self.__status = "Cancelled"
...
class Database:
    ...
    def _seed_defaults(self):
        ...
        for name, lat, lon, t in [
            ...
            cur.execute("INSERT OR IGNORE INTO locations (name,lat,lon,loc_type) VALUES(?,?,?,?)",
                        (name, lat, lon, t))
        self._conn.commit()
    ...
    def get_all_locations(self) -> list:
        return self._conn.execute(
            "SELECT * FROM locations ORDER BY loc_type, name").fetchall()

    def save_booking(self, b: Booking):
        self._conn.execute(
            "INSERT INTO bookings (booking_id,username,vehicle_id,pickup,status) VALUES(?,?,?,?,?)",
            (b.booking_id, b.user.username, b.vehicle.id, b.pickup.name, b.status))
        self._conn.commit()
    ...
    def get_bookings(self, username: str = None) -> list:
        if username:
            return self._conn.execute(
                ...
                (username,)).fetchall()
        return self._conn.execute(
            "SELECT * FROM bookings ORDER BY created_at DESC").fetchall()
...
class AppController:
    ...
    def _init_fleet(self) -> list:
        ...
        for orig, dest, cls in self.ROUTE_DEFS:
            try:
                fleet.append(cls(self._locations[orig], self._locations[dest]))
    ...
    def login(self, username: str, password: str) -> User:
        ...
        # Reconstruct User without calling __init__ again
    ...
    def book_vehicle(self, vehicle_id: str, pickup_name: str) -> Booking:
        ...
        if vehicle.is_full:
            raise VehicleCapacityError(f"{vehicle.vehicle_type} {vehicle_id} is full.")
        if pickup_name not in self._locations:
            raise LocationNotFoundError(f"'{pickup_name}' not found.")
        booking = Booking(self._current_user, vehicle, self._locations[pickup_name])
    ...
    def all_to_surigao(self) -> list:
        ...
        results = [(n, loc.loc_type, round(loc.distance_to(surigao), 2))
                   for n, loc in self._locations.items() if n != "Surigao City"]
    ...
    def distances_to(self, dest_name: str, origins: list) -> list:
        ...
        results = [(o, round(self._locations[o].distance_to(dest), 2))
                   for o in origins if o in self._locations and o != dest_name]
...
BG    = "#0B1120"; BG2   = "#111D30"; CARD  = "#182540"; CARD2 = "#1E2F50"
ACCENT= "#00C9FF"; GREEN = "#38EF7D"; ORANGE= "#FF8C42"; RED   = "#FF3B3B"
GOLD  = "#FFD700"; WHITE = "#FFFFFF"; DIM   = "#5C7A9A"
FBG   = ("Segoe UI",22,"bold"); FH = ("Segoe UI",12,"bold")
FN    = ("Segoe UI",10);        FS = ("Segoe UI", 9)
FC    = ("Courier New",9)
VCOLOR = {"Jeepney":GOLD,"Multicab":GREEN,"Bus":ACCENT}
LCOLOR = {"City":GOLD,"Municipality":DIM,"Barangay":"#88AACC","Landmark":ORANGE}
...
class AppView(tk.Tk):
    ...
    def _style(self):
        ...
        s.configure("Treeview", background=CARD, foreground=WHITE,
                    fieldbackground=CARD, rowheight=25, font=("Segoe UI",9))
    ...
    def _show_login(self):
        self._clear(); self._style()
        wrap = tk.Frame(self, bg=BG)
        wrap.place(relx=.5, rely=.5, anchor="center")
        tk.Label(wrap, text="🚌", font=("Segoe UI",52), bg=BG, fg=ACCENT).pack()
        tk.Label(wrap, text="Commuters Tracking System", font=FBG, bg=BG, fg=WHITE).pack(pady=(0,4))
        tk.Label(wrap, text="St. Paul University Surigao  ·  Surigao del Norte",
                 font=FN, bg=BG, fg=DIM).pack(pady=(0,20))
        card = tk.Frame(wrap, bg=BG2, padx=44, pady=34); card.pack()
        self._auth_tab = tk.StringVar(value="login")
        tab_row = tk.Frame(card, bg=BG2); tab_row.pack(fill="x", pady=(0,18))
        ...
        self._auth_body = tk.Frame(card, bg=BG2); self._auth_body.pack()
        self._build_login_form()
        tk.Label(wrap, text="Demo: simbahon/pass123  ·  dryan/pass456  ·  admin/admin123",
    ...
    def _build_login_form(self):
        for w in self._auth_body.winfo_children(): w.destroy()
        f = self._auth_body
        ...
        self._ep = tk.Entry(f, font=FN, bg=CARD, fg=WHITE,
                            insertbackground=WHITE, show="●", relief="flat", width=28)
        self._ep.pack(pady=(2,6), ipady=7)
        self._err_lbl = tk.Label(f, text="", font=FS, bg=BG2, fg=RED)
        self._err_lbl.pack()
        tk.Button(f, text="SIGN IN", font=FH, bg=ACCENT, fg=BG, relief="flat",
                  cursor="hand2", width=24, command=self._do_login
    ...
    def _build_register_form(self):
        for w in self._auth_body.winfo_children(): w.destroy()
        f = self._auth_body
        for lbl, attr, show in [("New Username","_ru",""),("Password","_rp","●"),("Confirm","_rc","●")]:
            tk.Label(f, text=lbl, font=FS, bg=BG2, fg=DIM).pack(anchor="w")
            e = tk.Entry(f, font=FN, bg=CARD, fg=WHITE, insertbackground=WHITE,
                         show=show, relief="flat", width=28)
            e.pack(pady=(2,10), ipady=7)
            setattr(self, attr, e)
        self._err_lbl = tk.Label(f, text="", font=FS, bg=BG2, fg=RED)
        self._err_lbl.pack()
        tk.Button(f, text="REGISTER", font=FH, bg=GREEN, fg=BG,
                  relief="flat", cursor="hand2", width=24, command=self._do_register
                  ).pack(pady=(8,0), ipady=7)

    def _do_login(self):
        try:
            u,p = self._eu.get().strip(), self._ep.get()
            if not u or not p: raise ValueError("Please fill in all fields.")
    ...
    def _do_register(self):
        try:
            u,p,p2 = self._ru.get().strip(), self._rp.get(), self._rc.get()
            if not u or not p: raise ValueError("Please fill in all fields.")
            if p != p2: raise ValueError("Passwords do not match.")
    ...
    def _show_main(self):
        ...
        top = tk.Frame(self, bg=BG2, height=52); top.pack(fill="x"); top.pack_propagate(False)
        tk.Label(top, text="🚌  Commuters Transportation Tracking System",
        ...
        col = ORANGE if user.is_admin else GREEN
        tk.Label(top, text=f"👤 {user.username}  [{user.role}]",
                 font=FN, bg=BG2, fg=col).pack(side="right", padx=6)

        tabs = ["🗺️ Map","📊 Distances","📋 Vehicles","🚀 Book Now","🎫 My Bookings"]
        if user.is_admin: tabs.append("🔧 Admin")
        tab_bar = tk.Frame(self, bg=BG, pady=5); tab_bar.pack(fill="x", padx=10)
        self._tab_btns = {}
        keys = ["Map","Distances","Vehicles","BookNow","Bookings","Admin"]
        for label, key in zip(tabs, keys[:len(tabs)]):
    ...
    def _switch_tab(self, key):
        ...
        for k,b in self._tab_btns.items():
            b.config(bg=ACCENT if k==key else CARD, fg=BG if k==key else WHITE)
        for w in self._content.winfo_children(): w.destroy()
        {"Map":self._tab_map,"Distances":self._tab_distances,
         "Vehicles":self._tab_vehicles,"BookNow":self._tab_booknow,
         "Bookings":self._tab_bookings,"Admin":self._tab_admin
         }.get(key, lambda: None)()

    def _logout(self):
        self._ctrl.logout(); self._show_login()

    # ── TAB: MAP ──────────────────────────────────────────────────────────────
    def _tab_map(self):
        pane = tk.Frame(self._content, bg=BG); pane.pack(fill="both", expand=True)
        left = tk.Frame(pane, bg=BG2, width=290); left.pack(side="left", fill="y", padx=(0,8))
        left.pack_propagate(False)
        tk.Label(left, text="Filter", font=FH, bg=BG2, fg=WHITE).pack(anchor="w", padx=12, pady=(12,4))
        fbf = tk.Frame(left, bg=BG2); fbf.pack(fill="x", padx=8)
        self._fbtns = {}
        for vt in ["All","Jeepney","Multicab","Bus"]:
            ...
            self._fbtns[vt] = b
        tk.Label(left, text="Vehicles", font=FH, bg=BG2, fg=WHITE).pack(anchor="w", padx=12, pady=(10,4))
        sf = tk.Frame(left, bg=BG2); sf.pack(fill="both", expand=True, padx=8, pady=(0,8))
        sb = tk.Scrollbar(sf); sb.pack(side="right", fill="y")
        self._vlist = tk.Listbox(sf, font=FC, bg=CARD, fg=WHITE,
                                 selectbackground=ACCENT, selectforeground=BG,
                                 relief="flat", activestyle="none", yscrollcommand=sb.set)
        ...
        tk.Label(self._detail, text="Select a vehicle from the list to view details",
                 font=FN, bg=BG2, fg=DIM).pack(expand=True)
        self._refresh_vlist(); self._draw_map()

    def _set_filter(self, vt):
        self._filter.set(vt)
        for k,b in self._fbtns.items():
            b.config(bg=ACCENT if k==vt else CARD, fg=BG if k==vt else WHITE)
        self._refresh_vlist()
    ...
    def _refresh_vlist(self):
        ...
        for i,v in enumerate(fleet):
            ...
            line = f"{dot} {v.id} {v.vehicle_type:<8} {v.passengers:2d}/{v.capacity} ETA:{v.eta_minutes}m"
            self._vlist.insert(tk.END, line)
            if v.id==sel_id: new_sel=i
        if new_sel is not None: self._vlist.selection_set(new_sel)
    ...
    def _show_detail(self, v: Vehicle):
        ...
        self._sel_v = v
        hdr = tk.Frame(self._detail, bg=CARD2, padx=14, pady=8); hdr.pack(fill="x")
        ...
        tk.Label(hdr, text=f"● {v.status}", font=FH, bg=CARD2, fg=sc).pack(side="right")
        body = tk.Frame(self._detail, bg=BG2, padx=10, pady=6); body.pack(fill="both", expand=True)
        info = v.get_info()
        for i,(lbl,val,col) in enumerate([
            ("🛣️  From",info["From"],ACCENT),("📍  To",info["To"],ACCENT),
            ("📏  Distance",info["Route km"],WHITE),("⏱️  ETA",info["ETA"],GREEN),
            ("👥  Passengers",info["Passengers"],ORANGE),("📝  Note",info.get("Note",""),DIM)
        ]):
            ...
            tk.Label(t, text=val, font=("Segoe UI",11,"bold"), bg=CARD, fg=col).pack(anchor="w")
        ...
        bar_bg = tk.Frame(bf, bg="#0A1628", height=14); bar_bg.pack(fill="x", pady=2)
        bc = RED if v.is_full else (ORANGE if v.occupancy_pct>.75 else GREEN)
        tk.Frame(bar_bg, bg=bc, height=14).place(relx=0, rely=0, relwidth=v.occupancy_pct, height=14)
        tk.Label(bf, text=f"{int(v.occupancy_pct*100)}%", font=FS, bg=BG2, fg=bc).pack(anchor="e")
        if not self._ctrl.current_user.is_admin:
            tk.Button(body, text="🎫  Book This Vehicle", font=FN,
                      ...
                      command=lambda: self._book_popup(v)
                      ).grid(row=4, column=0, columnspan=2, padx=4, pady=(8,0), sticky="ew", ipady=6)

    def _book_popup(self, v: Vehicle):
        win = tk.Toplevel(self); win.title("Book Vehicle")
        win.geometry("360x270"); win.configure(bg=BG2); win.grab_set()
        tk.Label(win, text=f"Book  {v.get_icon()} {v.id}", font=FH, bg=BG2, fg=ACCENT).pack(pady=(18,4))
        tk.Label(win, text=f"{v.origin.name}  →  {v.destination.name}", font=FN, bg=BG2, fg=WHITE).pack()
        tk.Label(win, text="Select your pickup location:", font=FS, bg=BG2, fg=DIM).pack(pady=(14,4))
        pickup_var = tk.StringVar(value=v.origin.name)
        cb = ttk.Combobox(win, textvariable=pickup_var,
                          values=sorted(self._ctrl.locations.keys()),
                          state="readonly", font=FN, width=30)
        cb.pack(ipady=4)
        err = tk.Label(win, text="", font=FS, bg=BG2, fg=RED); err.pack()
        def confirm():
            try:
        ...
        tk.Button(win, text="Confirm Booking", font=FH, bg=GREEN, fg=BG,
                  relief="flat", cursor="hand2", command=confirm
                  ).pack(pady=(10,0), ipady=7, padx=30, fill="x")

    def _draw_map(self):
        if not hasattr(self,"_map_cv"): return
        cv = self._map_cv; cv.delete("all")
        W = cv.winfo_width() or 800; H = cv.winfo_height() or 390
        ...
        for name,loc in locs.items():
            ...
            col=LCOLOR[loc.loc_type]; r=5 if loc.loc_type=="City" else 3
            cv.create_oval(x-r,y-r,x+r,y+r, fill=col, outline="white", width=1)
            if loc.loc_type in ("City","Landmark"):
                cv.create_text(x+8,y, text=name, font=("Segoe UI",7), fill=col, anchor="w")
        ...
        for i,(lbl,col) in enumerate([("Jeepney",GOLD),("Multicab",GREEN),("Bus",ACCENT),("FULL",RED)]):
            cv.create_oval(8,H-52+i*12,16,H-44+i*12, fill=col, outline="")
            cv.create_text(20,H-48+i*12, text=lbl, font=("Segoe UI",7), fill=col, anchor="w")
        cv.create_text(W-4,4, text="LIVE MAP  ·  Surigao del Norte",
                       font=("Segoe UI",8), fill=DIM, anchor="ne")

    # ── TAB: DISTANCES ────────────────────────────────────────────────────────
    def _tab_distances(self):
        outer = tk.Frame(self._content, bg=BG); outer.pack(fill="both", expand=True)
        tk.Label(outer, text="📊  Distance Calculator", font=FBG, bg=BG, fg=WHITE).pack(anchor="w", pady=(8,10))
        nb = ttk.Notebook(outer); nb.pack(fill="both", expand=True)
        fA = tk.Frame(nb, bg=BG2); nb.add(fA, text="  All → Surigao City  ")
        rows = [(n,lt,f"{km:.2f} km",f"~{max(1,int(km/35*60))} min",f"~{max(1,int(km/60*60))} min")
                for n,lt,km in self._ctrl.all_to_surigao()]
        self._dist_table(fA,"Distance from all locations to Surigao City",
                         ("Location","Type","Distance","ETA Jeepney","ETA Bus"),rows)
        fB = tk.Frame(nb, bg=BG2); nb.add(fB, text="  → Pier 2  ")
        rows_B = [(o,f"{km:.2f} km",f"~{max(1,int(km/35*60))} min",f"~{max(1,int(km/40*60))} min")
                  for o,km in self._ctrl.distances_to("Pier 2 (Pantalan 2)",
                                                       ["Punta Bilar","Serna","Terminal (Brgy Luna)"])]
        self._dist_table(fB,"Distance to Pier 2 (Pantalan 2)",
                         ("From","Distance","ETA Jeepney","ETA Multicab"),rows_B)
        fC = tk.Frame(nb, bg=BG2); nb.add(fC, text="  → Terminal  ")
        rows_C = [(o,f"{km:.2f} km",f"~{max(1,int(km/35*60))} min",f"~{max(1,int(km/40*60))} min")
                  for o,km in self._ctrl.distances_to("Terminal (Brgy Luna)",
                                                       ["Placer","Sison","Mainit","Claver","Tagana-an","Bad-as"])]
        self._dist_table(fC,"Distance to Terminal (Brgy Luna)",
                         ("From","Distance","ETA Jeepney","ETA Multicab"),rows_C)

    def _dist_table(self, parent, title, cols, rows):
        tk.Label(parent, text=title, font=FH, bg=BG2, fg=ACCENT).pack(anchor="w", padx=14, pady=(10,6))
        ...
        for r in rows: tv.insert("",tk.END,values=r)
    ...
    def _tab_vehicles(self):
        outer = tk.Frame(self._content, bg=BG); outer.pack(fill="both", expand=True)
        tk.Label(outer, text="📋  Live Vehicle Fleet", font=FBG, bg=BG, fg=WHITE).pack(anchor="w", pady=(8,10))
        cols = ("ID","Type","Icon","From","To","Passengers","Capacity","Occupancy","ETA","Status")
        tv = ttk.Treeview(outer, columns=cols, show="headings")
        for c,w in zip(cols,[65,80,50,185,185,90,80,90,80,70]):
            tv.heading(c,text=c); tv.column(c,width=w,anchor="center")
        self._fleet_tv = tv; self._populate_fleet_tv()
    ...
    def _populate_fleet_tv(self):
        ...
        for v in self._ctrl.get_fleet():
            tv.insert("",tk.END, values=(v.id,v.vehicle_type,v.get_icon(),
                v.origin.name,v.destination.name,v.passengers,v.capacity,
                f"{int(v.occupancy_pct*100)}%",f"{v.eta_minutes}m",v.status))

    # ── TAB: BOOK NOW ─────────────────────────────────────────────────────────
    def _tab_booknow(self):
        outer = tk.Frame(self._content, bg=BG); outer.pack(fill="both", expand=True)
        tk.Label(outer, text="🚀  Book a Vehicle", font=FBG, bg=BG, fg=WHITE).pack(anchor="w", pady=(8,2))
        tk.Label(outer, text="Select a vehicle from the list then click BOOK NOW",
                 font=FN, bg=BG, fg=DIM).pack(anchor="w", pady=(0,10))
        fbar = tk.Frame(outer, bg=BG); fbar.pack(fill="x", pady=(0,8))
        tk.Label(fbar, text="Filter:", font=FS, bg=BG, fg=DIM).pack(side="left", padx=(0,6))
        self._bn_filter = tk.StringVar(value="All"); self._bn_fbtns = {}
        for vt in ["All","Jeepney","Multicab","Bus"]:
            b = tk.Button(fbar, text=vt, font=FS,
                          bg=ACCENT if vt=="All" else CARD,
                          fg=BG if vt=="All" else WHITE,
                          relief="flat", cursor="hand2",
                          command=lambda v=vt: self._bn_set_filter(v))
            b.pack(side="left", padx=3, ipady=4, ipadx=8)
            self._bn_fbtns[vt] = b
        pane = tk.Frame(outer, bg=BG); pane.pack(fill="both", expand=True)
        left = tk.Frame(pane, bg=BG2, width=430); left.pack(side="left", fill="y", padx=(0,10))
        left.pack_propagate(False)
        cols = ("","ID","Type","From → To","Seats Left","ETA","Status")
        tv = ttk.Treeview(left, columns=cols, show="headings", height=22)
        for c,w in zip(cols,[22,58,78,192,75,60,65]):
            tv.heading(c,text=c); tv.column(c,width=w,anchor="center")
        self._bn_tv = tv
        ...
        tv.bind("<<TreeviewSelect>>", self._bn_on_select)
        self._bn_populate()
        right = tk.Frame(pane, bg=BG2); right.pack(side="left", fill="both", expand=True)
        self._bn_right = right; self._bn_show_placeholder()

    def _bn_set_filter(self, vt):
        self._bn_filter.set(vt)
        for k,b in self._bn_fbtns.items():
            b.config(bg=ACCENT if k==vt else CARD, fg=BG if k==vt else WHITE)
        self._bn_populate()

    def _bn_populate(self):
        if not hasattr(self,"_bn_tv"): return
        tv = self._bn_tv; vt = self._bn_filter.get()
        for r in tv.get_children(): tv.delete(r)
        for v in self._ctrl.get_fleet(vt):
            seats = v.capacity - v.passengers
            route = f"{v.origin.name} → {v.destination.name}"
            tag   = "full" if v.is_full else ("busy" if v.occupancy_pct>.75 else "ok")
            tv.insert("",tk.END, iid=v.id, tags=(tag,),
                      values=(v.get_icon(),v.id,v.vehicle_type,route,
                              f"{seats} free",f"{v.eta_minutes}m",v.status))
        tv.tag_configure("full", foreground=RED)
        tv.tag_configure("busy", foreground=ORANGE)
        tv.tag_configure("ok",   foreground=GREEN)

    def _bn_on_select(self, _=None):
        if not hasattr(self,"_bn_tv"): return
        sel = self._bn_tv.selection()
        if not sel: return
        v = next((x for x in self._ctrl.get_fleet() if x.id==sel[0]), None)
        if v: self._bn_show_booking_form(v)

    def _bn_show_placeholder(self):
        for w in self._bn_right.winfo_children(): w.destroy()
        tk.Label(self._bn_right, text="👈  Select a vehicle\nfrom the list to book",
                 font=("Segoe UI",14), bg=BG2, fg=DIM, justify="center").pack(expand=True)

    def _bn_show_booking_form(self, v: Vehicle):
        for w in self._bn_right.winfo_children(): w.destroy()
        f = self._bn_right
        hdr = tk.Frame(f, bg=CARD2, padx=18, pady=12)
        hdr.pack(fill="x", padx=12, pady=(12,6))
        sc = RED if v.is_full else (ORANGE if v.occupancy_pct>.75 else GREEN)
        tk.Label(hdr, text=f"{v.get_icon()}  {v.id}  —  {v.vehicle_type}",
                 font=("Segoe UI",14,"bold"), bg=CARD2, fg=ACCENT).pack(side="left")
        tk.Label(hdr, text=f"● {v.status}", font=FH, bg=CARD2, fg=sc).pack(side="right")
        info_f = tk.Frame(f, bg=BG2, padx=12); info_f.pack(fill="x", pady=4)
        for i,(lbl,val) in enumerate([
            ("🛣️  Route",    f"{v.origin.name}  →  {v.destination.name}"),
            ("📏  Distance", f"{v.route_km:.2f} km"),
            ("⏱️  ETA",      f"~{v.eta_minutes} minutes"),
            ("👥  Seats Free",f"{v.capacity-v.passengers} of {v.capacity}"),
        ]):
            t = tk.Frame(info_f, bg=CARD, padx=12, pady=8)
            t.grid(row=i//2, column=i%2, padx=4, pady=3, sticky="nsew")
            info_f.columnconfigure(i%2, weight=1)
            tk.Label(t, text=lbl, font=FS, bg=CARD, fg=DIM).pack(anchor="w")
            tk.Label(t, text=val, font=("Segoe UI",11,"bold"), bg=CARD, fg=WHITE).pack(anchor="w")
        bf = tk.Frame(f, bg=BG2, padx=16); bf.pack(fill="x", pady=(4,10))
        tk.Label(bf, text="Occupancy", font=FS, bg=BG2, fg=DIM).pack(anchor="w")
        bar_bg = tk.Frame(bf, bg="#0A1628", height=16); bar_bg.pack(fill="x", pady=3)
        bc = RED if v.is_full else (ORANGE if v.occupancy_pct>.75 else GREEN)
        tk.Frame(bar_bg, bg=bc, height=16).place(relx=0, rely=0, relwidth=v.occupancy_pct, height=16)
        tk.Label(bf, text=f"{int(v.occupancy_pct*100)}% full", font=FS, bg=BG2, fg=bc).pack(anchor="e")
        pick_f = tk.Frame(f, bg=BG2, padx=16); pick_f.pack(fill="x", pady=(0,8))
        tk.Label(pick_f, text="📍  Your Pickup Location:", font=FH, bg=BG2, fg=WHITE).pack(anchor="w", pady=(0,6))
        pickup_var = tk.StringVar(value=v.origin.name)
        cb = ttk.Combobox(pick_f, textvariable=pickup_var,
                          values=sorted(self._ctrl.locations.keys()),
                          state="readonly", font=FN, width=36)
        cb.pack(anchor="w", ipady=5)
        err = tk.Label(f, text="", font=FS, bg=BG2, fg=RED); err.pack()
        is_admin = self._ctrl.current_user.is_admin
        def do_book():
            try:
                b = self._ctrl.book_vehicle(v.id, pickup_var.get())
                messagebox.showinfo("✅ Booking Confirmed!",
                    ...
                    f"Vehicle     :  {b.vehicle.id} ({b.vehicle.vehicle_type})\n"
                    f"Route       :  {b.vehicle.origin.name} → {b.vehicle.destination.name}\n"
                    f"Pickup      :  {b.pickup.name}\n"
                    f"ETA         :  ~{b.vehicle.eta_minutes} min\n\n"
                    f"Check '🎫 My Bookings' to view your booking.")
                self._bn_show_placeholder()
                self._bn_tv.selection_remove(self._bn_tv.selection())
            except (VehicleCapacityError, LocationNotFoundError,
                    AuthenticationError, ValueError) as e:
                err.config(text=str(e))
        tk.Button(f,
                  text="🚀  BOOK NOW" if not v.is_full else "🚫  Vehicle is Full",
                  font=("Segoe UI",13,"bold"),
                  bg=GREEN if not v.is_full and not is_admin else CARD,
                  fg=BG if not v.is_full and not is_admin else DIM,
                  relief="flat", cursor="hand2",
                  state="normal" if not v.is_full and not is_admin else "disabled",
                  command=do_book
                  ).pack(fill="x", padx=16, pady=(4,4), ipady=12)
        if is_admin:
            tk.Label(f, text="Admins cannot book vehicles.", font=FS, bg=BG2, fg=DIM).pack()

    # ── TAB: MY BOOKINGS 
    def _tab_bookings(self):
        outer = tk.Frame(self._content, bg=BG); outer.pack(fill="both", expand=True)
        tk.Label(outer, text="🎫  My Bookings", font=FBG, bg=BG, fg=WHITE).pack(anchor="w", pady=(8,10))
        cols = ("Booking ID","Vehicle","Pickup","Status","Date/Time")
        tv = ttk.Treeview(outer, columns=cols, show="headings", height=16)
        for c,w in zip(cols,[110,110,200,100,200]):
            tv.heading(c,text=c); tv.column(c,width=w,anchor="center")
        rows = self._ctrl.get_my_bookings()
        if rows:
            for r in rows:
                tv.insert("",tk.END,values=(r["booking_id"],r["vehicle_id"],
                                             r["pickup"],r["status"],r["created_at"]))
        else:
            tv.insert("",tk.END,values=("—","No bookings yet","—","—","—"))
        sb = ttk.Scrollbar(outer, orient="vertical", command=tv.yview)
        tv.configure(yscrollcommand=sb.set)
        tv.pack(side="left", fill="both", expand=True, pady=(0,8))
        sb.pack(side="left", fill="y", pady=(0,8))
    ...
    def _tab_admin(self):
        ...
        tv = ttk.Treeview(outer, columns=cols, show="headings", height=16)
        for c,w in zip(cols,[110,110,100,190,100,190]):
            tv.heading(c,text=c); tv.column(c,width=w,anchor="center")
        for r in self._ctrl.get_all_bookings():
            tv.insert("",tk.END,values=(r["booking_id"],r["username"],r["vehicle_id"],
                                         r["pickup"],r["status"],r["created_at"]))
        sb = ttk.Scrollbar(outer, orient="vertical", command=tv.yview)
        tv.configure(yscrollcommand=sb.set)
        tv.pack(side="left", fill="both", expand=True, pady=(0,8))
    ...
    def _tick(self):
        ...
        if self._tab == "Map":
            self._draw_map(); self._refresh_vlist()
            if hasattr(self,"_sel_v"): self._show_detail(self._sel_v)
        elif self._tab == "Vehicles":
            self._populate_fleet_tv()
        elif self._tab == "BookNow":
            self._bn_populate()
        self.after(900, self._tick)

    def _on_close(self):
        self._running = False; self._ctrl.close(); self.destroy()
